...
class ErrorBoundary extends Component {
  ...
  componentDidCatch(error, errorInfo) {
    // Critical error - log for debugging
...
function LoadingScreen() {
  ...
  return React.createElement('div', { className: 'loading-screen' },
    ...
    React.createElement('div', { className: 'brand-preview' }, config.brand.fullName)
  );
}
...
function OrbitalElements() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      // Smooth follow
      smoothX += (mouseX - smoothX) * config.animations.tentacleFollowSpeed;
      smoothY += (mouseY - smoothY) * config.animations.tentacleFollowSpeed;

      // Rotate
      currentAngle = (currentAngle + config.animations.orbitSpeed) % 360;

      const radius = config.animations.orbitRadius;
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      tentaclesRef.current.forEach((tentacle, i) => {
        ...
        const maxRotation = config.animations.tentacleMaxRotation;
...
// ============================================
// CONTACT CARD COMPONENT (with split-view wrapper)
// ============================================
function ContactCard() {
  const config = window.AppConfig;
  const sentences = config.messages;
    ...
    if (charIdx <= sentence.length) {
      ...
      const t = setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          setText("");
          setVisible(true);
        }, config.typewriter.fadeOutDuration);
      }, config.typewriter.sentencePause);
      return () => clearTimeout(t);
    }
  }, [charIdx, idx, visible]);
  ...
  const copyEmail = (e) => {
    ...
    if (navigator.clipboard) {
      navigator.clipboard.writeText(email).then(() => {
        setCopied(true);
        setTimeout(() => setCopied(false), config.animations.copyNotification);
      });
    } else {
  ...
  // Two completely separate top-level containers
  return React.createElement(React.Fragment, null,
    // Contact Card Container
    React.createElement('div', {
      className: isAIConnected ? 'contact-container shifted' : 'contact-container'
    },
      React.createElement('div', {
        className: 'board',
        id: "main-content",
    ...
    React.createElement('div', { className: "skateboard" },
      ...
      React.createElement('a', {
        ...
        'aria-label': `Email ${config.contact.name} at ${config.contact.email}. Right-click to copy email address`,
        onClick: e => { stopSpeech(); setTimeout(() => window.location.href = `mailto:${config.contact.email}`, 10); },
      ...
      React.createElement('div', {
        ...
        ['linkedin', 'leetcode', 'github'].map(social =>
          React.createElement('a', {
            key: social,
            className: "skate-btn",
            href: config.links[social],
            target: "_blank",
            rel: "noopener noreferrer",
            'aria-label': `Visit ${config.contact.name}'s ${social.charAt(0).toUpperCase() + social.slice(1)} profile (opens in new tab)`
    ...
    React.createElement('div', {
      className: isAIConnected ? 'chat-container active' : 'chat-container'
    },
      window.ChatWindow && React.createElement(window.ChatWindow, {
        voiceClient: voiceClientRef.current,
...
function App() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setLoading(false), config.animations.loadingDuration);
